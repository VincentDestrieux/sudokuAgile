...
public class PossibilitiesGrid {
    ...
    //retourne les valeurs du possibilitiesGrid
    public ArrayList<Integer> valeurPossibilitiesGrid(int i, int j) {
        return this.possibilitiesGrid[i][j];
    }
    ...
    //Retourne la valeur du singleton ou -1 s'il y a plusieurs possibilitées.
    ...
    public boolean LoneColumn(PairCoord pos) {
        //Initialise
        ArrayList<ArrayList<PairCoord>> run = initialiseTabLone();
        ...
        for (int i = 0; i < 9; i++) {
            if (!(this.isSingleton(new PairCoord(pos.getX(), i + 1)))) {
                for (Integer value : possibilitiesGrid[i][pos.getX() - 1])
                    run.get(value - 1).add(new PairCoord(pos.getX(), i + 1));
            }
        }
        //Now we check if some values appears only
        return checkTabLone(run);
    }
    ...
    public boolean LoneRow(PairCoord pos) {
        //Initialise
        ArrayList<ArrayList<PairCoord>> run = initialiseTabLone();
        //Run on row -> if a value appears only one if this column, it's the only option to solve this grid.
        for (int i = 0; i < 9; i++) {
            if (!(this.isSingleton(new PairCoord(i + 1, pos.getY())))) {
                for (Integer value : possibilitiesGrid[pos.getY() - 1][i]) {
                    run.get(value - 1).add(new PairCoord(i + 1, pos.getY()));
                }
            }
        }
        //Now we check if some values appears only
    ...
    public boolean LoneBlock(PairCoord pos) {
        ...
        ArrayList<ArrayList<PairCoord>> run = initialiseTabLone();
        PairCoord cornerAssociatedPos = PossibilitiesGrid.CornerSG.get(Grid.getSubGrid(pos));
        // run on block -> if a value appears only one in this block, it's the only option to solve this grid.
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                //IF SINGLETON
                if (!(this.isSingleton(new PairCoord(cornerAssociatedPos.getX() + j, cornerAssociatedPos.getY() + i)))) {
                    for (Integer value : possibilitiesGrid[cornerAssociatedPos.getY() + i - 1][cornerAssociatedPos.getX() + j - 1]) {
                        run.get(value - 1).add(new PairCoord(cornerAssociatedPos.getX() + j, cornerAssociatedPos.getY() + i));
                    }
                }
            }
        }
    ...
    private ArrayList<ArrayList<PairCoord>> initialiseTabLone() {
        ArrayList<ArrayList<PairCoord>> run = new ArrayList<>(9);
        for (int i = 0; i < 9; i++)
            run.add(new ArrayList<PairCoord>());
        return run;

    }

    private boolean checkTabLone(ArrayList<ArrayList<PairCoord>> run) {
        boolean change = false;
        int val = 1;
        for (ArrayList<PairCoord> numberOfAppears : run) {
            if (numberOfAppears.size() == 1) {
                this.update(numberOfAppears.get(0), val);
                change = true;
            }
            val++;
        }
